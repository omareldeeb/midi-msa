...
class ByMeasureTrack(object):
    ...
    def _cleanup_note_duplicates(self):
        ...
        for t in self.tracks_by_measure:
            ...
            new_note_ons = self._cleanup_note_duplicates_helper__compute_new_note_ons(note_ons=t.note_ons)
        ...
        # finally, sort the new note ons
        for t in self.tracks_by_measure:
            t.sort_note_ons()

    def _cleanup_note_duplicates_helper__compute_new_note_ons(self, note_ons):
...
def combine_tracks(L, inst_for_result=None) -> Track:
    ...
    name_for_result = L[0].name if L else ''
