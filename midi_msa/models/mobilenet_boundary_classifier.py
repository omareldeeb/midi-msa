...
from ..data.piano_roll_dataset import PianoRollDatasetItem
...
class MobileNetBoundaryClassifier(nn.Module):
    ...
    def forward(self, x: PianoRollDatasetItem) -> torch.Tensor:
        piano_roll_patch = x.piano_roll_patch
        sslm_patch = x.sslm_patch

        piano_roll_output = self.piano_roll_model(piano_roll_patch)
        if not hasattr(self, 'sslm_model'):
            return piano_roll_output

        sslm_output = self.sslm_model(sslm_patch)
